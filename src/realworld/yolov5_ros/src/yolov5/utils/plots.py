...
class Colors:
    ...
    def __init__(self):
        self.class_colors = {
            0 : (0, 0, 255),  # Blue color
            1: (255, 255, 0),  # Yellow color
        }

    def __call__(self, class_name, bgr=False):
        color = self.class_colors.get(class_name, (255, 255, 255))  # Default to white if class_name not found
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            self.font = check_pil_font(font='Arial.Unicode.ttf' if non_ascii else font,
                                       size=font_size or max(round(sum(self.im.size) / 2 * 0.0015), 3))
        else:  # use cv2
            self.im = im
        self.lw = line_width or max(round(sum(im.shape) / 2 * 0.00025), 1)  # line width
