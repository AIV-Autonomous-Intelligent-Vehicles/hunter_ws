...
class Yolov5Detector:
    ...
    def activate_yolo_service_callback(self, req):
        ...
        if req.roi.height == 1 or req.object_name.data == 'right':
            if self.compressed_input:
                ...
                header = self.data_r.header
        if req.roi.height == 2 or req.object_name.data == 'left':
            if self.compressed_input2:
        ...
        if len(det):
            ...
            for class_index, count in class_counts.items():
                ...
                if count > 1:
                    ...
                    plus_s = ''
                    # rs += f"{self.names[int(class_index)]}: {count} {plus_s}, "
                # rs += f"Right : {count} {class_name}{plus_s}, "
                
            # rs = rs[:-2]
            # print(rs + '\t')
            result_string += f"fps: {fps}\n"
